...
export default function App() {
  ...
  const { stock, setMonthlyProfitGoal, setYearlyProfitGoal } = useStockStore()
  ...
  return (
    ...
    <div className="w-full flex flex-col bg-white md:bg-[#f0f4fb] md:min-h-screen"
      ...
      <main className="flex-1 overflow-y-auto md:overflow-visible">
        {activeTab === 'dashboard' && <DashboardPage reports={activeReports} stockProducts={stock.products} taxRate={stock.taxRate} monthlyProfitGoal={stock.monthlyProfitGoal} onSetMonthlyGoal={setMonthlyProfitGoal} yearlyProfitGoals={stock.yearlyProfitGoals} onSetYearlyGoal={setYearlyProfitGoal} activeBranch={selectedSite} setActiveBranch={setSelectedSite} syncStatus={syncStatus} lastSynced={lastSynced ?? undefined} categoryAliases={stock.categoryAliases} />}
        {activeTab === 'stock' && (
          <StockPage
            reports={allReports}
            sheetsUrl={sheetsUrl}
